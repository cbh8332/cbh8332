...
def bing_search_mobile(cookies, account_index=None, email=None):
    ...
    # 保留关键的cookie字段
    essential_fields = ['MUID', 'MUIDB', '_EDGE_V', '_SS', 'SRCHD', 'SRCHUSR', 'SRCHHPGUSR', '_RwBf']
    ...
    # 添加移动端特定的cookie设置
    essential_cookies.extend([
        'SRCHUSR=DOB=20250706',
        'SRCHD=AF=NOFORM', 
        'SRCHHPGUSR=SRCHLANG=zh-Hans&DM=1&CW=360&CH=640&SCW=360&SCH=640&BRW=MW&BRH=MS&DPR=2.0&UTC=480&HV=' + str(int(time.time()))
    ])
    
    enhanced_cookies = '; '.join(essential_cookies)
    
    url = "https://cn.bing.com/search"
    params = {
        "q": q,
        "form": "QBRE",  # 改为标准移动端form参数
        "sp": "-1",
        "lq": "0",
        "pq": q[:10] if len(q) > 10 else q,
        "sc": "10-0",
        "qs": "n",
        "sk": "",
        "cvid": "".join(random.choices('ABCDEF0123456789', k=32)),
        "cc": "CN",
        "setlang": "zh-hans"
    }
    ...
    headers = {
        ...
        "Sec-Fetch-User": "?1",
        "Upgrade-Insecure-Requests": "1",
        "User-Agent": "Mozilla/5.0 (Linux; Android 10; SM-G973F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Mobile Safari/537.36 EdgA/121.0.0.0",
    ...
    try:
        ...
        if response.status_code == 200:
            # 检查响应内容是否包含搜索结果标识
            if 'b_searchbox' in response.text or 'b_results' in response.text:
                return True
            else:
                print_log("移动搜索", "搜索请求成功但可能未被识别为移动搜索", account_index)
                return False
        else:
            print_log("移动搜索", f"搜索请求失败，状态码: {response.status_code}", account_index)
            return False
    except Exception as e:
...
def perform_search_tasks(search_type, search_func, cookies, account_index=None):
    """搜索任务执行函数，增加更多调试信息"""
    max_searches = 25  # 增加最大搜索次数
    check_interval = 5  # 每5次检查一次进度
    
    print_log(search_type, f"开始执行{search_type}任务", account_index)
    
    # 获取初始进度
    dashboard_result = get_dashboard_data(cookies, account_index)
    if not dashboard_result:
        print_log(search_type, "无法获取dashboard数据", account_index)
        return
    ...
    # 检查任务是否已完成
    if progress_type == 'pcSearch' and is_pc_search_complete(dashboard_data):
        print_log(search_type, "电脑搜索任务已完成", account_index)
        return
    elif progress_type == 'mobileSearch' and is_mobile_search_complete(dashboard_data):
        print_log(search_type, "移动搜索任务已完成", account_index)
        return
    
    # 显示初始进度
    initial_progress = get_search_progress_sum(dashboard_data, progress_type)
    print_log(search_type, f"初始进度: {initial_progress}", account_index)
    ...
    for i in range(max_searches):
        count += 1
        success = search_func(cookies, account_index)
        
        if success:
            delay = random.randint(15, 25)
            print_log(search_type, f"第 {count} 次搜索成功，等待 {delay} 秒...", account_index)
            time.sleep(delay)
        else:
            print_log(search_type, f"第 {count} 次搜索失败", account_index)
            time.sleep(5)
        ...
        if count % check_interval == 0:
            ...
            if dashboard_result:
                dashboard_data = dashboard_result['dashboard_data']
                current_progress = get_search_progress_sum(dashboard_data, progress_type)
                progress_change = current_progress - last_progress
                
                print_log(search_type, f"已完成 {count} 次，进度: {last_progress} -> {current_progress} (+{progress_change})", account_index)
                ...
                # 如果连续多次搜索后进度没有变化，可能有问题
                if count >= 15 and progress_change == 0:
                    print_log(search_type, "搜索多次但进度无变化，可能存在问题", account_index)
    
    # 最终检查
    dashboard_result = get_dashboard_data(cookies, account_index)
    if dashboard_result:
        dashboard_data = dashboard_result['dashboard_data']
        final_progress = get_search_progress_sum(dashboard_data, progress_type)
        total_progress = final_progress - initial_progress
        print_log(search_type, f"搜索任务结束，总进度变化: +{total_progress}", account_index)
    ...
    for i in range(check_interval):
        ...
        if progress_type == 'pcSearch':
            ...
            if is_mobile_search_complete(dashboard_data):
                ...
                break
    # 4次后中止线程
    if progress_type == 'pcSearch' and not is_pc_search_complete(dashboard_data):
        print_log(f"{search_type}", f"{check_interval}次后PC搜索未完成，停止线程", account_index)
        search_thread_stopped.set()
        raise SystemExit()

    if progress_type == 'mobileSearch' and not is_mobile_search_complete(dashboard_data):
        print_log(f"{search_type}", f"{check_interval}次后移动搜索未完成，继续执行其他任务", account_index)
    return
